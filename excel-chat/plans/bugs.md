...
## Bug 18: Sanitized Upload Returns 0 Sheets (S3 Re-parse Fails)
...
## Bug 19: Fallback Model Confuses `retrieve` and `retrieve_batch` Tools
...
## Bug 20: Test Files Still Importing Removed `retrieve` Function After Tool Merge

**Date:** 2026-08-01 (CI failure), fixed 2026-08-05

**Symptom:**
CI Backend Tests job failed with `ImportError` and `NameError` on the `agentic` branch after the `retrieve`/`retrieve_batch` merge into `retrieve_values` (Bug 19). All 5 consecutive CI runs failed.
...
## Bug 21: Short-Circuit Retrieves Wrong Field for Hierarchical Items
...
## Bug 22: Semantic Cache Returns Wrong Answers for Different Year Ranges
...
The semantic cache matched too aggressively — queries with different year ranges (e.g. "2015-2020" vs "2020-2023") would hit the cache at 97%+ semantic similarity and return the wrong cached response. Additionally, stale cache entries from Bug 21 (containing wrong values and empty `friendly_response`) persisted and kept being served even after the item parsing fix was deployed.
...
**Cache flush** (`main.py`): Added `POST /cache/clear` endpoint to invalidate all cached responses for a user. Used to flush 57 stale entries containing wrong values from Bug 21.
