...
## Planned: Semantic Caching (Per-User)
...
```
User Query
  → Embed query (TF-IDF or lightweight embedding model)
  → Search user's cache namespace for cosine similarity > threshold
  → Hit (sim > 0.92)? → Return cached response
  → Miss? → Call LLM → Store {embedding, query, response} in user's namespace
```
...
2. **Embedding model for cache keys**
   - **Selected**: `Qwen/Qwen3-Embedding-8B` via `sentence-transformers`
   - 1024 dimensions (truncated), state-of-the-art semantic quality
   - Handles complex financial queries with domain-specific terminology
   - Long-term viable — won't need replacing as query complexity grows
   - Usage:
     ```python
     from sentence_transformers import SentenceTransformer
     model = SentenceTransformer("Qwen/Qwen3-Embedding-8B", truncate_dim=1024)
...
4. **Create `semantic_cache.py`**
   ...
   def _get_model():
       global _model
       if _model is None:
           _model = SentenceTransformer("Qwen/Qwen3-Embedding-8B", truncate_dim=1024)
       return _model
...
5. **Storage: Redis with vector search**
   ...
   - Index: `FT.CREATE idx ON JSON PREFIX 1 semantic_cache: SCHEMA $.embedding VECTOR FLAT 6 TYPE FLOAT32 DIM 1024 DISTANCE_METRIC COSINE`
...
7. **Threshold tuning**
   - Start at 0.92 (conservative — fewer false positives)
   - Log cache hit similarity scores to monitor
   - A/B test: 0.85 vs 0.90 vs 0.92 — measure user satisfaction with cached responses
...
### Testing
- Unit: `embed_query()` returns 1024-dim normalized vector
...
### Cost Estimate
- `Qwen/Qwen3-Embedding-8B`: free, open-source, runs on GPU or CPU (slower on CPU)
- Model size: ~16GB download (one-time), loads from local cache after first use
- Redis Stack: same as Redis plan above (1024-dim vectors = ~4KB per cache entry)
...
| Layer | What's Cached | Key | Scope | Storage |
|-------|--------------|-----|-------|---------|
| **Exact-match** | Sheet auto-description LLM responses | `sha256(model:prompt)` | Global (not per-user) | Redis `llm:{hash}` / SQLite `llm_cache` |
| **Semantic** | Full query results (final answer + step_results) | `semantic:{user_id}:{sha256(model:query)}` | Per-user | Redis hash / SQLite `llm_cache` with `embedding_json` |

**Gap**: The semantic cache stores the *entire query response* as one blob. When a user asks "total revenue 2022-2025" then "ratio of revenue to grants 2022-2025", the second query is a cache miss because the queries aren't semantically similar enough (0.92 threshold). The pipeline re-retrieves revenue from scratch even though it was already computed.

### Planned: Intermediate Result Cache
...
1. **Embed the description** using `embed_query()` (same Qwen3 model)
